...
void KrpcProvider::Run() {
  ...
  // service_name为永久节点，method_name节点
  for (auto &sp : service_map) {
    ...
    for (auto &mp : sp.second.method_map) {
      ...
      // method_path 作为方法目录节点，永久保存所有 service 实例的临时子节点
      zkclient.Create(method_path.c_str(), nullptr, 0);

      std::string instance_path =
          method_path + "/" + ip + ":" + std::to_string(port);
      char method_path_data[128] = {0};
      sprintf(method_path_data, "%s:%d", ip.c_str(),
              port); // 将IP和端口信息存入节点数据
      // ZOO_EPHEMERAL表示这个节点是临时节点，在服务断开连接后，ZooKeeper会自动删除该实例节点
      zkclient.Create(instance_path.c_str(), method_path_data,
...
void KrpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn,
  ...
  while (buffer->readableBytes() >= 4) {
    ...
    google::protobuf::Closure *done =
        google::protobuf::NewCallback<KrpcProvider,
